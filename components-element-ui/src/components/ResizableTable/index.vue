<template>
  <table>
    <colgroup>
      <col v-for="col in columns" :key="col.value" :style="columnWidths[col.value] ? { width: columnWidths[col.value] + 'px' } : {}" />
    </colgroup>
    <thead>
      <tr>
        <th v-for="col in columns" :key="col.value">
          <slot name="header" :column="col">
            <template v-if="col.options && col.options.length">
              <MyDropdown :value="filter[col.filterValue || col.value]" @command="(value) => emitFilterChange(value, col)" :options="col.options"> </MyDropdown>
            </template>
            <template v-else>
              {{ col.label }}
            </template>
