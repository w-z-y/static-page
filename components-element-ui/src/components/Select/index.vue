<template>
  <el-select popper-class="my-select-popper" ref="mySelectRef" :class="['my-select']" v-bind="$attrs" :multiple="multiple" clearable collapse-tags v-model="internalValue" @change="handleChangeValue" @visible-change="handleVisibleChange" @clear="handleClear">
    <template v-if="type === 'tree'">
      <el-option v-show="false" :label="selectedNode[defaultProps.label]" :value="selectedNode[defaultProps.value]"> </el-option>
      <MyTree ref="tree" @node-click="handleNodeClick" :expand-on-click-node="expandOnClickNode" check-strictly :node-key="defaultProps.value" :default-checked-keys="defaultCheckedKeys" :props="defaultProps" default-expand-all :data="options"> </MyTree>
    </template>
    <template v-else>
      <el-option v-for="option in options" :key="option[defaultProps.value]" :label="option[defaultProps.label]" :value="option[defaultProps.value]">
        <MyTooltip>
